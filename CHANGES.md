...
- HTML sustainability review: `<strike>` → `<del>`; `<font color="...">` → `<span class="...">`; block-level `<add>` content wrapped in `<div>` rather than `<span>`; nested `<del>` rendered as `<span class="del">`; `<a name="...">` → `<a id="...">`; all `<img>` tags given `alt` attributes; page break anchor ids prefixed with `p`

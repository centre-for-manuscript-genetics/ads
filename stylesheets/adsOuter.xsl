...
<table class="main" align="center" border="0" cellspacing="0"><xsl:if test="$export!='print'"><xsl:attribute name="width">802</xsl:attribute></xsl:if><xsl:if test="$export='print'"><xsl:attribute name="width">100%</xsl:attribute></xsl:if>
 ...
 <xsl:if test="$comp1!=''">[<xsl:if test="not(substring($comp1,4,1) = 'Z')"><xsl:value-of select="$comp1"/></xsl:if><xsl:if test="substring($comp1,4,1) = 'Z'"><xsl:value-of select="substring($comp1,1,3)"/><xsl:value-of select="substring($comp1,5,2)"/></xsl:if> vs. <xsl:if test="not(substring($comp2,4,1) = 'Z')"><xsl:value-of select="$comp2"/></xsl:if><xsl:if test="substring($comp2,4,1) = 'Z'"><xsl:value-of select="substring($comp2,1,3)"/><xsl:value-of select="substring($comp2,5,2)"/></xsl:if>]</xsl:if></td><xsl:if test="$export!='print'"><td><div><xsl:attribute name="class"><xsl:if test="substring($document,1,3) = 'Ads'">navigationright2</xsl:if><xsl:if test="substring($document,1,3) = 'Opd' and $text='doclincomp'">navigationright2</xsl:if></xsl:attribute><xsl:if test="$text != 'doclinlay' and $text != 'doclincomp' and substring($document,1,3) = 'Ads'"><p class="navigation"><img alt="dot" src="images/navi/dot.jpg"/><xsl:text> </xsl:text><a><!-- STATIC CONVERSION 2026-03-10: new url mapping --><xsl:attribute name="href"><xsl:if test="not(substring($document,4,1) = 'Z')"><xsl:value-of select="$document"/></xsl:if><xsl:if test="substring($document,4,1) = 'Z'"><xsl:value-of select="substring($document,1,3)"/><xsl:value-of select="substring($document,5,2)"/>a</xsl:if>-zinsvarianten.html</xsl:attribute>toon schrijfproces per zin</a></p></xsl:if><xsl:if test="$text = 'doclinlay' and substring($document,1,3) = 'Ads' and $n=''"><p class="navigation"><img alt="dot" src="images/navi/dot.jpg"/><xsl:text> </xsl:text>Klik op '<img alt="dot" src="images/navi/dot.jpg"/>' voor de zinsvarianten</p></xsl:if>
 ...
 <div class="inside">
  ...
  <xsl:if test="$comp1!=''">[<xsl:if test="not(substring($comp1,4,1) = 'Z')"><xsl:value-of select="$comp1"/></xsl:if><xsl:if test="substring($comp1,4,1) = 'Z'"><xsl:value-of select="substring($comp1,1,3)"/><xsl:value-of select="substring($comp1,5,2)"/></xsl:if> vs. <xsl:if test="not(substring($comp2,4,1) = 'Z')"><xsl:value-of select="$comp2"/></xsl:if><xsl:if test="substring($comp2,4,1) = 'Z'"><xsl:value-of select="substring($comp2,1,3)"/><xsl:value-of select="substring($comp2,5,2)"/></xsl:if>]<br/></xsl:if></td><xsl:if test="$export!='print'"><td><div><xsl:attribute name="class"><xsl:if test="substring($document,1,3) = 'Ads'">navigationright2</xsl:if><xsl:if test="substring($document,1,3) = 'Opd' and $text='doclincomp'">navigationright2</xsl:if></xsl:attribute><xsl:if test="$text != 'doclinlay' and $text != 'doclincomp' and substring($document,1,3) = 'Ads'"><p class="navigation"><img alt="dot" src="images/navi/dot.jpg"/><xsl:text> </xsl:text><a><xsl:attribute name="href"><xsl:if test="not(substring($document,4,1) = 'Z')"><xsl:value-of select="$document"/></xsl:if><xsl:if test="substring($document,4,1) = 'Z'"><!-- STATIC CONVERSION 2026-03-10: new url mapping --><xsl:value-of select="substring($document,1,3)"/><xsl:value-of select="substring($document,5,2)"/>a</xsl:if>-zinsvarianten.html</xsl:attribute>toon schrijfproces per zin</a></p></xsl:if><xsl:if test="$text = 'doclinlay' and substring($document,1,3) = 'Ads' and $n=''"><p class="navigation"><img alt="dot" src="images/navi/dot.jpg"/><xsl:text> </xsl:text>Klik op '<img alt="dot" src="images/navi/dot.jpg"/>' voor de zinsvarianten</p></xsl:if><xsl:if test="$text='doclincomp'"><xsl:call-template name="doclincomp"/></xsl:if></div></td></xsl:if></tr></table></xsl:if>
   <xsl:apply-templates select="descendant::text[@id= concat(substring($document,1,3), 'DD')]"/>
 </div>
 </xsl:if>
 <!-- de facs en zo -->
